...
public class UserServlet extends HttpServlet {
    ...
    private static final String FILE_PATH = "C/Users/ruslannikonov/IdeaProjects/lab/src/main/java/user.json"; // Путь к файлу на сервере
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // Запись списка пользователей обратно в файл
        try (FileWriter fileWriter = new FileWriter(FILE_PATH)) {
            gson.toJson(users, fileWriter);
            System.out.println("abc6");
        } catch (IOException e) {
            e.printStackTrace();
            response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Ошибка при записи студента в файл");
            return;
        }
